data.h=read.csv("D:/h.csv",header = T)
head(data.h)
...
ggplot(data=total.d,aes(x=Attention,y=Result)) + geom_point(shape="*",size=8,color="black")+stat_smooth(method = 'lm',se=F,color="red")
ggplot(data=total.d, aes(x=Attention,y=Result)) + geom_jitter(width = 0.5, height = 0.5,shape="*",size=8)+stat_smooth(method = 'lm',se=F,color="red")

library(MASS)
plot(R~A,pch="*",cex=2)
abline(lqs(R~A,method = "lts"),col="green3")
abline(lqs(R~A,method = "lms"),col="red")
abline(lm(R~A),col="blue")


summary(lm(R~A))

logic=glm(R~Group,data=total.d)
...
aov(total.d$Result~total.d$Group)
summary(aov(total.d$Result~total.d$Group))
bartlett.test(total.d$Result~total.d$Group)


anova(lm(R~group,data=total.d))
install.packages("agricolae")
library(agricolae)
duncan.test(aov(R~Group,data=total.d), "Group", alpha = 0.05, console = TRUE)
out=LSD.test(aov(R~Group,data=total.d),"Group", p.adj = "bonferroni",group = T)
out
#plot
